...
class UsuariosAdminController extends AppController {
    ...
    public function delete($id = null)
    {
        ...
        if(isset($id)){
            ...
            if($usuario != null){
                $usuario->Ativo = false;
                $this->UsuariosAdmin->save($usuario);
    ...
    public function mudarSenha()
    {
        if ($this->request->is(['post', 'put'])) {
            ...
            $usuario = $this->UsuariosAdmin->find()->where(['Codigo' => $this->request->session()->read('Auth.Admin.id')])->first();
            $senhaAtualValida = (new DefaultPasswordHasher)->check($senhaAntiga,$usuario->Senha);
